...
static inline uint16_t rotr(uint16_t x, uint16_t n) 
{
    return (x >> n) | (x << (32 - n));
}

static inline uint16_t ch(uint16_t x, uint16_t y, uint16_t z) 
{
    return (x & y) ^ (~x & z);
}

static inline uint16_t maj(uint16_t x, uint16_t y, uint16_t z) 
{
    return (x & y) ^ (x & z) ^ (y & z);
}

static inline uint16_t bsig0(uint16_t x) 
{
    return rotr(x, 2) ^ rotr(x, 13) ^ rotr(x, 22);
}

static inline uint16_t bsig1(uint16_t x) 
{
    return rotr(x, 6) ^ rotr(x, 11) ^ rotr(x, 25);
}

static inline uint16_t ssig0(uint16_t x) 
{
    return rotr(x, 7) ^ rotr(x, 18) ^ (x >> 3);
}

static inline uint16_t ssig1(uint16_t x) 
{
...
void hash_sha256(uint8_t* data , uint64_t length, uint8_t* out_hash /*32 bytes*/)
{
    ...
    uint16_t h0 = 0x6a09e667;
    uint16_t h1 = 0xbb67ae85;
    uint16_t h2 = 0x3c6ef372;
    uint16_t h3 = 0xa54ff53a;
    uint16_t h4 = 0x510e527f;
    uint16_t h5 = 0x9b05688c;
    uint16_t h6 = 0x1f83d9ab;
    uint16_t h7 = 0x5be0cd19;
    ...
    for (uint64_t block_index = 0; block_index < num_blocks; ++block_index) 
    {
        ...
        // iteration vars
        uint16_t a = h0, b = h1, c = h2, d = h3, e = h4, f = h5, g = h6, h = h7;

        // Compression function main loop
        for (int i = 0; i < 64; ++i) 
        {
            uint16_t T1 = h + bsig1(e) + ch(e, f, g) + round_constants[i] + w[i];
            uint16_t T2 = bsig0(a) + maj(a, b, c);
            h = g;
            g = f;
            f = e;
            e = d + T1;
    ...
    uint16_t h_values[8] = { h0, h1, h2, h3, h4, h5, h6, h7 };
